...
BOOST_FIXTURE_TEST_CASE( buysell, eosio_system_tester ) try {
   ...
   // confirm alice have 0.
   BOOST_REQUIRE_EQUAL( core_sym::from_string("0.00000000"), get_balance( "alice1111111" ) );

   // alice have 1000
   transfer( "eosio", "alice1111111", core_sym::from_string("1000.00000000"), "eosio" );

   // alice stake 200 , 100 for cpu and net to call contract.
   BOOST_REQUIRE_EQUAL( success(), stake( "eosio", "alice1111111", core_sym::from_string("200.00000000"), core_sym::from_string("100.00000000") ) );
   ...
   // alice buy 200 ram.
   BOOST_REQUIRE_EQUAL( success(), buyram( "alice1111111", "alice1111111", core_sym::from_string("200.00000000") ) );

   // confirm alice only have 800 now.
   BOOST_REQUIRE_EQUAL( core_sym::from_string("800.00000000"), get_balance( "alice1111111" ) );

   // confirm eosio.ram increate 199,  and remfee increase 1.
   BOOST_REQUIRE_EQUAL( initial_ram_balance + core_sym::from_string("199.00000000"), get_balance(N(eosio.ram)) );
   BOOST_REQUIRE_EQUAL( initial_ramfee_balance + core_sym::from_string("1.00000000"), get_balance(N(eosio.ramfee)) );

   // dump the ram alice bought,  and sell all.
   ...
   // confirm alice bought and sold 200 ram,  bi-direction changed .5% , now balance is 998.00499999
   BOOST_REQUIRE_EQUAL( core_sym::from_string("998.00499999"), b );
   total = get_total_stake( "alice1111111" );

   // confirm alice's total ram still as initials. ( 0 )
   ...
   // confirm alice have 10000 + 998.00499999
   ...
   // confirm after alice sold all bought,  the ram still same.
   BOOST_REQUIRE_EQUAL( true, total["ram_bytes"].as_uint64() == init_bytes );
   // confirm alice balanece after buy and sell.
   BOOST_REQUIRE_EQUAL( core_sym::from_string("10988.02999995"), get_balance( "alice1111111" ) );

   // alice buy 560 in 9 txs.
   ...
   // confirm alice 9 txs charged right.
   ...
   // alice sell all 5060 bought.
   BOOST_REQUIRE_EQUAL( success(), sellram( "alice1111111", bought_bytes ) );

   // confirm rammarket is not empty.
   BOOST_REQUIRE_EQUAL( false, get_row_by_account( config::system_account_name, config::system_account_name,
                                                   N(rammarket), account_name(symbol{SY(CORE_SYM_PRECISION,RAMCORE)}.value()) ).empty() );

   // get ram market lambda.
   auto get_ram_market = [this]() -> fc::variant {
      vector<char> data = get_row_by_account( config::system_account_name, config::system_account_name,
                                              N(rammarket), account_name(symbol{SY(CORE_SYM_PRECISION,RAMCORE)}.value()) );
      BOOST_REQUIRE( !data.empty() );
      return abi_ser.binary_to_variant("exchange_state", data, abi_serializer_max_time);
   };

   // test bancor calculation.
   {
      transfer( config::system_account_name, N(alice1111111), core_sym::from_string("240000.00000000"), config::system_account_name );
      ...
      const auto supply = market["supply"];

      BOOST_REQUIRE_EQUAL( asset::from_string("0 RAM").get_symbol(),     r0.get_symbol() );
      BOOST_REQUIRE_EQUAL( core_sym::from_string("0.00000000").get_symbol(), e0.get_symbol() );

      const asset payment = core_sym::from_string("240000.00000000");
      // alice buy 1000 ram.
      ...
      // confirm alice bought ram amount matech bancor calc.
      BOOST_REQUIRE_EQUAL( expected_delta, bytes1 -  bytes0 );
      wdump((supply)(r0.get_amount())(e0.get_amount()));
   }

   {
      // bob have 24K
      transfer( config::system_account_name, N(bob111111111), core_sym::from_string("24000.00000000"), config::system_account_name );
      // bob buy ram failed. ( buy 1 will still charge 0.5% , which rounding to 0)
      BOOST_REQUIRE_EQUAL( wasm_assert_msg("must reserve a positive amount"),buyrambytes( "bob111111111", "bob111111111", 1 ) );
...
BOOST_FIXTURE_TEST_CASE( stake_unstake, eosio_system_tester ) try {
   ...
   BOOST_REQUIRE_EQUAL( core_sym::from_string("1000.00000000"), get_balance( "alice1111111" ) );
   BOOST_REQUIRE_EQUAL( success(), stake( "eosio", "alice1111111", core_sym::from_string("200.00000000"), core_sym::from_string("100.00000000") ) );

   auto total = get_total_stake("alice1111111");
   BOOST_REQUIRE_EQUAL( core_sym::from_string("210.00000000"), total["net_weight"].as<asset>());
   BOOST_REQUIRE_EQUAL( core_sym::from_string("110.00000000"), total["cpu_weight"].as<asset>());

   const auto init_eosio_stake_balance = get_balance( N(eosio.stake) );
   BOOST_REQUIRE_EQUAL( success(), stake( "alice1111111", "alice1111111", core_sym::from_string("200.00000000"), core_sym::from_string("100.00000000") ) );
   BOOST_REQUIRE_EQUAL( core_sym::from_string("700.00000000"), get_balance( "alice1111111" ) );
   BOOST_REQUIRE_EQUAL( init_eosio_stake_balance + core_sym::from_string("300.00000000"), get_balance( N(eosio.stake) ) );
   BOOST_REQUIRE_EQUAL( success(), unstake( "alice1111111", "alice1111111", core_sym::from_string("200.00000000"), core_sym::from_string("100.00000000") ) );
   BOOST_REQUIRE_EQUAL( core_sym::from_string("700.00000000"), get_balance( "alice1111111" ) );
   ...
   BOOST_REQUIRE_EQUAL( success(), stake( "alice1111111", "bob111111111", core_sym::from_string("200.00000000"), core_sym::from_string("100.00000000") ) );
   BOOST_REQUIRE_EQUAL( core_sym::from_string("700.00000000"), get_balance( "alice1111111" ) );
   ...
   total = get_total_stake( "alice1111111" );
   BOOST_REQUIRE_EQUAL( core_sym::from_string("210.00000000").get_amount(), total["net_weight"].as<asset>().get_amount() );
   BOOST_REQUIRE_EQUAL( core_sym::from_string("110.00000000").get_amount(), total["cpu_weight"].as<asset>().get_amount() );

   REQUIRE_MATCHING_OBJECT( voter( "alice1111111", core_sym::from_string("300.00000000")), get_voter_info( "alice1111111" ) );

   auto bytes = total["ram_bytes"].as_uint64();
   BOOST_REQUIRE_EQUAL( true, 0 < bytes );

   //unstake from bob111111111
   BOOST_REQUIRE_EQUAL( success(), unstake( "alice1111111", "bob111111111", core_sym::from_string("200.00000000"), core_sym::from_string("100.00000000") ) );
   total = get_total_stake("bob111111111");
   BOOST_REQUIRE_EQUAL( core_sym::from_string("10.00000000"), total["net_weight"].as<asset>());
   BOOST_REQUIRE_EQUAL( core_sym::from_string("10.00000000"), total["cpu_weight"].as<asset>());
   produce_block( fc::hours(3*24-1) );
   produce_blocks(1);
   BOOST_REQUIRE_EQUAL( core_sym::from_string("700.00000000"), get_balance( "alice1111111" ) );
   //after 3 days funds should be released
   ...
   REQUIRE_MATCHING_OBJECT( voter( "alice1111111", core_sym::from_string("0.00000000") ), get_voter_info( "alice1111111" ) );
   produce_blocks(1);
   BOOST_REQUIRE_EQUAL( core_sym::from_string("1000.00000000"), get_balance( "alice1111111" ) );
} FC_LOG_AND_RETHROW()
...
BOOST_FIXTURE_TEST_CASE( stake_unstake_with_transfer, eosio_system_tester ) try {
   ...
   transfer( "eosio", "bob111111111", core_sym::from_string("1000.00000000"), "eosio" );
   // stake transfer from bob to alice, 200 net , 100 cpu.
   BOOST_REQUIRE_EQUAL( success(), stake_with_transfer( N(bob111111111), N(alice1111111), core_sym::from_string("200.00000000"), core_sym::from_string("100.00000000") ) );

   //check that alice has both bandwidth and voting power
   auto total = get_total_stake("alice1111111");
   BOOST_REQUIRE_EQUAL( core_sym::from_string("210.00000000"), total["net_weight"].as<asset>());
   BOOST_REQUIRE_EQUAL( core_sym::from_string("110.00000000"), total["cpu_weight"].as<asset>());
   // confirm alice 's staking.
   REQUIRE_MATCHING_OBJECT( voter( "alice1111111", core_sym::from_string("300.00000000")), get_voter_info( "alice1111111" ) );
   // confirm alice's balence still 0.
   BOOST_REQUIRE_EQUAL( core_sym::from_string("0.00000000"), get_balance( "alice1111111" ) );

   // alice stakes for herself
   // alice have 1000.
   transfer( "eosio", "alice1111111", core_sym::from_string("1000.00000000"), "eosio" );
   // alice stake 200 net , 100 cpu.
   BOOST_REQUIRE_EQUAL( success(), stake( "alice1111111", "alice1111111", core_sym::from_string("200.00000000"), core_sym::from_string("100.00000000") ) );
   //now alice's stake should be equal to transfered from eosio + own stake
   total = get_total_stake("alice1111111");
   // alice's balance 1000 -300 = 700
   // alice's stake net 200+210 = 410.
   // alice's stake cpu 100+110 = 210.
   BOOST_REQUIRE_EQUAL( core_sym::from_string("700.00000000"), get_balance( "alice1111111" ) );
   BOOST_REQUIRE_EQUAL( core_sym::from_string("410.00000000"), total["net_weight"].as<asset>());
   BOOST_REQUIRE_EQUAL( core_sym::from_string("210.00000000"), total["cpu_weight"].as<asset>());
   // alice 's total vote = 410+210 = 620
   REQUIRE_MATCHING_OBJECT( voter( "alice1111111", core_sym::from_string("600.00000000")), get_voter_info( "alice1111111" ) );

   //alice can unstake everything (including what was transfered)
   BOOST_REQUIRE_EQUAL( success(), unstake( "alice1111111", "alice1111111", core_sym::from_string("400.00000000"), core_sym::from_string("200.00000000") ) );
   // confirm alice's balence : 700.
   BOOST_REQUIRE_EQUAL( core_sym::from_string("700.00000000"), get_balance( "alice1111111" ) );

   produce_block( fc::hours(3*24-1) );
   produce_blocks(1);
   // confirm just before 3 days. alice's balence still 700.
   BOOST_REQUIRE_EQUAL( core_sym::from_string("700.00000000"), get_balance( "alice1111111" ) );
   //after 3 days funds should be released
   produce_block( fc::hours(1) );
   produce_blocks(1);
   // confirm after 3 days, alice's stake released.
   BOOST_REQUIRE_EQUAL( core_sym::from_string("1300.00000000"), get_balance( "alice1111111" ) );

   //stake should be equal to what was staked in constructor, voting power should be 0
   total = get_total_stake("alice1111111");
   // confirm residual is 10 in net and cpu.
   BOOST_REQUIRE_EQUAL( core_sym::from_string("10.00000000"), total["net_weight"].as<asset>());
   BOOST_REQUIRE_EQUAL( core_sym::from_string("10.00000000"), total["cpu_weight"].as<asset>());
   // confirm 0 voting 
   REQUIRE_MATCHING_OBJECT( voter( "alice1111111", core_sym::from_string("0.00000000")), get_voter_info( "alice1111111" ) );
   ...
   BOOST_REQUIRE_EQUAL( success(), stake_with_transfer( N(alice1111111), N(bob111111111), core_sym::from_string("100.00000000"), core_sym::from_string("100.00000000") ) );
...
BOOST_FIXTURE_TEST_CASE( stake_to_self_with_transfer, eosio_system_tester ) try {
   ...
   // alice have 1000.
   transfer( "eosio", "alice1111111", core_sym::from_string("1000.00000000"), "eosio" );
   // confirm can't stake to self.
   BOOST_REQUIRE_EQUAL( wasm_assert_msg("cannot use transfer flag if delegating to self"),
                        stake_with_transfer( N(alice1111111), N(alice1111111), core_sym::from_string("200.00000000"), core_sym::from_string("100.00000000") )
   );
...
BOOST_FIXTURE_TEST_CASE( stake_while_pending_refund, eosio_system_tester ) try {
   ...
   // bob have 1000.
   transfer( "eosio", "bob111111111", core_sym::from_string("1000.00000000"), "eosio" );
   // bob stake transfer to alice, 200, 100
   BOOST_REQUIRE_EQUAL( success(), stake_with_transfer( N(bob111111111), N(alice1111111), core_sym::from_string("200.00000000"), core_sym::from_string("100.00000000") ) );
...
BOOST_FIXTURE_TEST_CASE( fail_without_auth, eosio_system_tester ) try {
   ...
   issue_and_transfer( "alice1111111", core_sym::from_string("1000.00000000"),  config::system_account_name );

   BOOST_REQUIRE_EQUAL( success(), stake( "eosio", "alice1111111", core_sym::from_string("2000.00000000"), core_sym::from_string("1000.00000000") ) );
   BOOST_REQUIRE_EQUAL( success(), stake( "alice1111111", "bob111111111", core_sym::from_string("10.00000000"), core_sym::from_string("10.00000000") ) );
   ...
   BOOST_REQUIRE_EQUAL( error("missing authority of alice1111111"),
                        push_action( N(alice1111111), N(delegatebw), mvo()
                                    ...
                                    ("stake_net_quantity", core_sym::from_string("10.00000000"))
                                    ("stake_cpu_quantity", core_sym::from_string("10.00000000"))
                                    ("transfer", 0 )
                                    ,false
                        )
   );
   ...
   BOOST_REQUIRE_EQUAL( error("missing authority of alice1111111"),
                        push_action(N(alice1111111), N(undelegatebw), mvo()
                                    ("from",     "alice1111111")
                                    ("receiver", "bob111111111")
                                    ("unstake_net_quantity", core_sym::from_string("200.00000000"))
                                    ("unstake_cpu_quantity", core_sym::from_string("100.00000000"))
                                    ("transfer", 0 )
                                    ,false
                        )
   );
...
BOOST_FIXTURE_TEST_CASE( stake_negative, eosio_system_tester ) try {
   ...
   BOOST_REQUIRE_EQUAL( wasm_assert_msg("must stake a positive amount"),
                        stake( "alice1111111", core_sym::from_string("-0.00010000"), core_sym::from_string("0.00000000") )
   );

   BOOST_REQUIRE_EQUAL( wasm_assert_msg("must stake a positive amount"),
                        stake( "alice1111111", core_sym::from_string("0.00000000"), core_sym::from_string("-0.00010000") )
   );

   BOOST_REQUIRE_EQUAL( wasm_assert_msg("must stake a positive amount"),
                        stake( "alice1111111", core_sym::from_string("00.00000000"), core_sym::from_string("00.00000000") )
   );

   BOOST_REQUIRE_EQUAL( wasm_assert_msg("must stake a positive amount"),
                        stake( "alice1111111", core_sym::from_string("0.00000000"), core_sym::from_string("00.00000000") )
...
BOOST_FIXTURE_TEST_CASE( unstake_negative, eosio_system_tester ) try {
   ...
   BOOST_REQUIRE_EQUAL( success(), stake( "alice1111111", "bob111111111", core_sym::from_string("200.00010000"), core_sym::from_string("100.00010000") ) );

   auto total = get_total_stake( "bob111111111" );
   BOOST_REQUIRE_EQUAL( core_sym::from_string("210.00010000"), total["net_weight"].as<asset>());
   auto vinfo = get_voter_info("alice1111111" );
   wdump((vinfo));
   REQUIRE_MATCHING_OBJECT( voter( "alice1111111", core_sym::from_string("300.00020000") ), get_voter_info( "alice1111111" ) );


   BOOST_REQUIRE_EQUAL( wasm_assert_msg("must unstake a positive amount"),
                        unstake( "alice1111111", "bob111111111", core_sym::from_string("-1.00000000"), core_sym::from_string("0.00000000") )
   );

   BOOST_REQUIRE_EQUAL( wasm_assert_msg("must unstake a positive amount"),
                        unstake( "alice1111111", "bob111111111", core_sym::from_string("0.00000000"), core_sym::from_string("-1.00000000") )
   );

   //unstake all zeros
   BOOST_REQUIRE_EQUAL( wasm_assert_msg("must unstake a positive amount"),
                        unstake( "alice1111111", "bob111111111", core_sym::from_string("0.00000000"), core_sym::from_string("0.00000000") )
...
BOOST_FIXTURE_TEST_CASE( unstake_more_than_at_stake, eosio_system_tester ) try {
   ...
   issue_and_transfer( "alice1111111", core_sym::from_string("1000.00000000"),  config::system_account_name );
   BOOST_REQUIRE_EQUAL( success(), stake( "alice1111111", core_sym::from_string("200.00000000"), core_sym::from_string("100.00000000") ) );
   ...
   BOOST_REQUIRE_EQUAL( wasm_assert_msg("insufficient staked net bandwidth"),
                        unstake( "alice1111111", core_sym::from_string("200.00010000"), core_sym::from_string("0.00000000") )
   );

   //trying to unstake more cpu bandwith than at stake
   BOOST_REQUIRE_EQUAL( wasm_assert_msg("insufficient staked cpu bandwidth"),
                        unstake( "alice1111111", core_sym::from_string("0.00000000"), core_sym::from_string("100.00010000") )
...
BOOST_FIXTURE_TEST_CASE( delegate_to_another_user, eosio_system_tester ) try {
   ...
   BOOST_REQUIRE_EQUAL( wasm_assert_msg("insufficient staked cpu bandwidth"),
                        unstake( "bob111111111", core_sym::from_string("0.00000000"), core_sym::from_string("10.00000000") )

   );
   BOOST_REQUIRE_EQUAL( wasm_assert_msg("insufficient staked net bandwidth"),
                        unstake( "bob111111111", core_sym::from_string("10.00000000"),  core_sym::from_string("0.00000000") )
   );

   issue_and_transfer( "carol1111111", core_sym::from_string("1000.00000000"),  config::system_account_name );
   BOOST_REQUIRE_EQUAL( success(), stake( "carol1111111", "bob111111111", core_sym::from_string("20.00000000"), core_sym::from_string("10.00000000") ) );
   total = get_total_stake( "bob111111111" );
   BOOST_REQUIRE_EQUAL( core_sym::from_string("230.00000000"), total["net_weight"].as<asset>());
   BOOST_REQUIRE_EQUAL( core_sym::from_string("120.00000000"), total["cpu_weight"].as<asset>());
   BOOST_REQUIRE_EQUAL( core_sym::from_string("970.00000000"), get_balance( "carol1111111" ) );
   REQUIRE_MATCHING_OBJECT( voter( "carol1111111", core_sym::from_string("30.00000000") ), get_voter_info( "carol1111111" ) );

   //alice1111111 should not be able to unstake money staked by carol1111111

   BOOST_REQUIRE_EQUAL( wasm_assert_msg("insufficient staked net bandwidth"),
                        unstake( "alice1111111", "bob111111111", core_sym::from_string("2001.00000000"), core_sym::from_string("1.00000000") )
   );

   BOOST_REQUIRE_EQUAL( wasm_assert_msg("insufficient staked cpu bandwidth"),
                        unstake( "alice1111111", "bob111111111", core_sym::from_string("1.00000000"), core_sym::from_string("101.00000000") )
...
BOOST_FIXTURE_TEST_CASE( stake_unstake_separate, eosio_system_tester ) try {
   ...
   //everything at once
   BOOST_REQUIRE_EQUAL( success(), stake( "alice1111111", core_sym::from_string("10.00000000"), core_sym::from_string("20.00000000") ) );
   auto total = get_total_stake( "alice1111111" );
   BOOST_REQUIRE_EQUAL( core_sym::from_string("20.00000000"), total["net_weight"].as<asset>());
   BOOST_REQUIRE_EQUAL( core_sym::from_string("30.00000000"), total["cpu_weight"].as<asset>());

   //cpu
   BOOST_REQUIRE_EQUAL( success(), stake( "alice1111111", core_sym::from_string("100.00000000"), core_sym::from_string("0.00000000") ) );
   total = get_total_stake( "alice1111111" );
   BOOST_REQUIRE_EQUAL( core_sym::from_string("120.00000000"), total["net_weight"].as<asset>());
   BOOST_REQUIRE_EQUAL( core_sym::from_string("30.00000000"), total["cpu_weight"].as<asset>());

   //net
   BOOST_REQUIRE_EQUAL( success(), stake( "alice1111111", core_sym::from_string("0.00000000"), core_sym::from_string("200.00000000") ) );
   total = get_total_stake( "alice1111111" );
   BOOST_REQUIRE_EQUAL( core_sym::from_string("120.00000000"), total["net_weight"].as<asset>());
   BOOST_REQUIRE_EQUAL( core_sym::from_string("230.00000000"), total["cpu_weight"].as<asset>());

   //unstake cpu
   BOOST_REQUIRE_EQUAL( success(), unstake( "alice1111111", core_sym::from_string("100.00000000"), core_sym::from_string("0.00000000") ) );
   total = get_total_stake( "alice1111111" );
   BOOST_REQUIRE_EQUAL( core_sym::from_string("20.00000000"), total["net_weight"].as<asset>());
   BOOST_REQUIRE_EQUAL( core_sym::from_string("230.00000000"), total["cpu_weight"].as<asset>());

   //unstake net
   BOOST_REQUIRE_EQUAL( success(), unstake( "alice1111111", core_sym::from_string("0.00000000"), core_sym::from_string("200.00000000") ) );
   total = get_total_stake( "alice1111111" );
   BOOST_REQUIRE_EQUAL( core_sym::from_string("20.00000000"), total["net_weight"].as<asset>());
   BOOST_REQUIRE_EQUAL( core_sym::from_string("30.00000000"), total["cpu_weight"].as<asset>());
} FC_LOG_AND_RETHROW()
...
BOOST_FIXTURE_TEST_CASE( adding_stake_partial_unstake, eosio_system_tester ) try {
   ...
   BOOST_REQUIRE_EQUAL( success(), stake( "alice1111111", "bob111111111", core_sym::from_string("100.00000000"), core_sym::from_string("50.00000000") ) );

   auto total = get_total_stake( "bob111111111" );
   BOOST_REQUIRE_EQUAL( core_sym::from_string("310.00000000"), total["net_weight"].as<asset>());
   BOOST_REQUIRE_EQUAL( core_sym::from_string("160.00000000"), total["cpu_weight"].as<asset>());
   REQUIRE_MATCHING_OBJECT( voter( "alice1111111", core_sym::from_string("450.00000000") ), get_voter_info( "alice1111111" ) );
   BOOST_REQUIRE_EQUAL( core_sym::from_string("550.00000000"), get_balance( "alice1111111" ) );

   //unstake a share
   BOOST_REQUIRE_EQUAL( success(), unstake( "alice1111111", "bob111111111", core_sym::from_string("150.00000000"), core_sym::from_string("75.00000000") ) );

   total = get_total_stake( "bob111111111" );
   BOOST_REQUIRE_EQUAL( core_sym::from_string("160.00000000"), total["net_weight"].as<asset>());
   BOOST_REQUIRE_EQUAL( core_sym::from_string("85.00000000"), total["cpu_weight"].as<asset>());
   REQUIRE_MATCHING_OBJECT( voter( "alice1111111", core_sym::from_string("225.00000000") ), get_voter_info( "alice1111111" ) );

   //unstake more
   BOOST_REQUIRE_EQUAL( success(), unstake( "alice1111111", "bob111111111", core_sym::from_string("50.00000000"), core_sym::from_string("25.00000000") ) );
   total = get_total_stake( "bob111111111" );
   BOOST_REQUIRE_EQUAL( core_sym::from_string("110.00000000"), total["net_weight"].as<asset>());
   BOOST_REQUIRE_EQUAL( core_sym::from_string("60.00000000"), total["cpu_weight"].as<asset>());
   REQUIRE_MATCHING_OBJECT( voter( "alice1111111", core_sym::from_string("150.00000000") ), get_voter_info( "alice1111111" ) );
   ...
   produce_blocks(1);
   BOOST_REQUIRE_EQUAL( core_sym::from_string("850.00000000"), get_balance( "alice1111111" ) );
...
BOOST_FIXTURE_TEST_CASE( stake_from_refund, eosio_system_tester ) try {
   ...
   BOOST_REQUIRE_EQUAL( success(), stake( "alice1111111", "bob111111111", core_sym::from_string("50.00000000"), core_sym::from_string("50.00000000") ) );

   total = get_total_stake( "bob111111111" );
   BOOST_REQUIRE_EQUAL( core_sym::from_string("60.00000000"), total["net_weight"].as<asset>());
   BOOST_REQUIRE_EQUAL( core_sym::from_string("60.00000000"), total["cpu_weight"].as<asset>());

   REQUIRE_MATCHING_OBJECT( voter( "alice1111111", core_sym::from_string("400.00000000") ), get_voter_info( "alice1111111" ) );
   BOOST_REQUIRE_EQUAL( core_sym::from_string("600.00000000"), get_balance( "alice1111111" ) );

   //unstake a share
   BOOST_REQUIRE_EQUAL( success(), unstake( "alice1111111", "alice1111111", core_sym::from_string("100.00000000"), core_sym::from_string("50.00000000") ) );
   total = get_total_stake( "alice1111111" );
   BOOST_REQUIRE_EQUAL( core_sym::from_string("110.00000000"), total["net_weight"].as<asset>());
   BOOST_REQUIRE_EQUAL( core_sym::from_string("60.00000000"), total["cpu_weight"].as<asset>());
   REQUIRE_MATCHING_OBJECT( voter( "alice1111111", core_sym::from_string("250.00000000") ), get_voter_info( "alice1111111" ) );
   BOOST_REQUIRE_EQUAL( core_sym::from_string("600.00000000"), get_balance( "alice1111111" ) );
   auto refund = get_refund_request( N(alice1111111) );
   BOOST_REQUIRE_EQUAL( core_sym::from_string("100.00000000"), refund["net_amount"].as<asset>() );
   BOOST_REQUIRE_EQUAL( core_sym::from_string( "50.00000000"), refund["cpu_amount"].as<asset>() );
   ...
   BOOST_REQUIRE_EQUAL( core_sym::from_string("110.00000000"), total["net_weight"].as<asset>());
   BOOST_REQUIRE_EQUAL( core_sym::from_string("60.00000000"), total["cpu_weight"].as<asset>());
   REQUIRE_MATCHING_OBJECT( voter( "alice1111111", core_sym::from_string("350.00000000") ), get_voter_info( "alice1111111" ) );
   BOOST_REQUIRE_EQUAL( core_sym::from_string("500.00000000"), get_balance( "alice1111111" ) );
   refund = get_refund_request( N(alice1111111) );
   BOOST_REQUIRE_EQUAL( core_sym::from_string("100.00000000"), refund["net_amount"].as<asset>() );
   BOOST_REQUIRE_EQUAL( core_sym::from_string( "50.00000000"), refund["cpu_amount"].as<asset>() );

   //stake less than pending refund, entire amount should be taken from refund
   BOOST_REQUIRE_EQUAL( success(), stake( "alice1111111", "alice1111111", core_sym::from_string("50.00000000"), core_sym::from_string("25.00000000") ) );
   total = get_total_stake( "alice1111111" );
   BOOST_REQUIRE_EQUAL( core_sym::from_string("160.00000000"), total["net_weight"].as<asset>());
   BOOST_REQUIRE_EQUAL( core_sym::from_string("85.00000000"), total["cpu_weight"].as<asset>());
   refund = get_refund_request( N(alice1111111) );
   BOOST_REQUIRE_EQUAL( core_sym::from_string("50.00000000"), refund["net_amount"].as<asset>() );
   BOOST_REQUIRE_EQUAL( core_sym::from_string("25.00000000"), refund["cpu_amount"].as<asset>() );
   ...
   BOOST_REQUIRE_EQUAL( core_sym::from_string("500.00000000"), get_balance( "alice1111111" ) );
   refund = get_refund_request( N(alice1111111) );
   BOOST_REQUIRE_EQUAL( core_sym::from_string("200.00000000"), refund["net_amount"].as<asset>() );
   BOOST_REQUIRE_EQUAL( core_sym::from_string("100.00000000"), refund["cpu_amount"].as<asset>() );

   //stake more than pending refund
   BOOST_REQUIRE_EQUAL( success(), stake( "alice1111111", "alice1111111", core_sym::from_string("300.00000000"), core_sym::from_string("200.00000000") ) );
   total = get_total_stake( "alice1111111" );
   BOOST_REQUIRE_EQUAL( core_sym::from_string("310.00000000"), total["net_weight"].as<asset>());
   BOOST_REQUIRE_EQUAL( core_sym::from_string("210.00000000"), total["cpu_weight"].as<asset>());
   REQUIRE_MATCHING_OBJECT( voter( "alice1111111", core_sym::from_string("700.00000000") ), get_voter_info( "alice1111111" ) );
   refund = get_refund_request( N(alice1111111) );
   BOOST_TEST_REQUIRE( refund.is_null() );
   //200 core tokens should be taken from alice's account
   BOOST_REQUIRE_EQUAL( core_sym::from_string("300.00000000"), get_balance( "alice1111111" ) );
...
BOOST_FIXTURE_TEST_CASE( stake_to_another_user_not_from_refund, eosio_system_tester ) try {
   ...
   //unstake
   BOOST_REQUIRE_EQUAL( success(), unstake( "alice1111111", core_sym::from_string("200.00000000"), core_sym::from_string("100.00000000") ) );
   auto refund = get_refund_request( N(alice1111111) );
   BOOST_REQUIRE_EQUAL( core_sym::from_string("200.00000000"), refund["net_amount"].as<asset>() );
   BOOST_REQUIRE_EQUAL( core_sym::from_string("100.00000000"), refund["cpu_amount"].as<asset>() );
   ...
   BOOST_REQUIRE_EQUAL( core_sym::from_string("400.00000000"), get_balance( "alice1111111" ) );
   refund = get_refund_request( N(alice1111111) );
   BOOST_REQUIRE_EQUAL( core_sym::from_string("200.00000000"), refund["net_amount"].as<asset>() );
   BOOST_REQUIRE_EQUAL( core_sym::from_string("100.00000000"), refund["cpu_amount"].as<asset>() );
...
BOOST_FIXTURE_TEST_CASE( producer_wtmsig, eosio_system_tester ) try {
   ...
   issue_and_transfer( N(alice1111111), core_sym::from_string("20000.00000000"),  config::system_account_name );
   ...
   BOOST_REQUIRE_EQUAL( success(), stake( N(alice1111111), core_sym::from_string("10000.00000000"), core_sym::from_string("10000.00000000") ) );
...
BOOST_FIXTURE_TEST_CASE( vote_for_producer, eosio_system_tester, * boost::unit_test::tolerance(1e+5) ) try {
   ...
   issue_and_transfer( "bob111111111", core_sym::from_string("2000.00000000"),  config::system_account_name );
   issue_and_transfer( "carol1111111", core_sym::from_string("3000.00000000"),  config::system_account_name );

   //bob111111111 makes stake
   BOOST_REQUIRE_EQUAL( success(), stake( "bob111111111", core_sym::from_string("11.00000000"), core_sym::from_string("0.11110000") ) );
   BOOST_REQUIRE_EQUAL( core_sym::from_string("1988.88890000"), get_balance( "bob111111111" ) );
   REQUIRE_MATCHING_OBJECT( voter( "bob111111111", core_sym::from_string("11.11110000") ), get_voter_info( "bob111111111" ) );
   ...
   BOOST_TEST_REQUIRE( stake2votes(core_sym::from_string("11.11110000")) == prod["total_votes"].as_double() );
   BOOST_REQUIRE_EQUAL( "alice1111111", prod["owner"].as_string() );
   BOOST_REQUIRE_EQUAL( "http://block.one", prod["url"].as_string() );

   //carol1111111 makes stake
   BOOST_REQUIRE_EQUAL( success(), stake( "carol1111111", core_sym::from_string("22.00000000"), core_sym::from_string("0.22220000") ) );
   REQUIRE_MATCHING_OBJECT( voter( "carol1111111", core_sym::from_string("22.22220000") ), get_voter_info( "carol1111111" ) );
   BOOST_REQUIRE_EQUAL( core_sym::from_string("2977.77780000"), get_balance( "carol1111111" ) );
   ...
   prod = get_producer_info( "alice1111111" );
   BOOST_TEST_REQUIRE( stake2votes(core_sym::from_string("33.33330000")) == prod["total_votes"].as_double() );

   //bob111111111 increases his stake
   BOOST_REQUIRE_EQUAL( success(), stake( "bob111111111", core_sym::from_string("33.00000000"), core_sym::from_string("0.33330000") ) );
   //alice1111111 stake with transfer to bob111111111
   BOOST_REQUIRE_EQUAL( success(), stake_with_transfer( N(alice1111111), N(bob111111111), core_sym::from_string("22.00000000"), core_sym::from_string("0.22220000") ) );
   //should increase alice1111111's total_votes
   prod = get_producer_info( "alice1111111" );
   BOOST_TEST_REQUIRE( stake2votes(core_sym::from_string("88.88880000")) == prod["total_votes"].as_double() );

   //carol1111111 unstakes part of the stake
   BOOST_REQUIRE_EQUAL( success(), unstake( "carol1111111", core_sym::from_string("2.00000000"), core_sym::from_string("0.00020000")/*"2.00000000 EOS", "0.00020000 EOS"*/ ) );

   //should decrease alice1111111's total_votes
   prod = get_producer_info( "alice1111111" );
   wdump((prod));
   BOOST_TEST_REQUIRE( stake2votes(core_sym::from_string("86.88860000")) == prod["total_votes"].as_double() );
   ...
   //should decrease alice1111111's total_votes
   prod = get_producer_info( "alice1111111" );
   BOOST_TEST_REQUIRE( stake2votes(core_sym::from_string("20.22200000")) == prod["total_votes"].as_double() );
   //but eos should still be at stake
   BOOST_REQUIRE_EQUAL( core_sym::from_string("1955.55560000"), get_balance( "bob111111111" ) );

   //carol1111111 unstakes rest of eos
   BOOST_REQUIRE_EQUAL( success(), unstake( "carol1111111", core_sym::from_string("20.00000000"), core_sym::from_string("0.22200000") ) );
   ...
   produce_block();
   BOOST_REQUIRE_EQUAL( core_sym::from_string("3000.00000000"), get_balance( "carol1111111" ) );
...
BOOST_FIXTURE_TEST_CASE( unregistered_producer_voting, eosio_system_tester, * boost::unit_test::tolerance(1e+5) ) try {
   issue_and_transfer( "bob111111111", core_sym::from_string("2000.00000000"),  config::system_account_name );
   BOOST_REQUIRE_EQUAL( success(), stake( "bob111111111", core_sym::from_string("13.00000000"), core_sym::from_string("0.57910000") ) );
   REQUIRE_MATCHING_OBJECT( voter( "bob111111111", core_sym::from_string("13.57910000") ), get_voter_info( "bob111111111" ) );
...
BOOST_FIXTURE_TEST_CASE( vote_same_producer_30_times, eosio_system_tester ) try {
   issue_and_transfer( "bob111111111", core_sym::from_string("2000.00000000"),  config::system_account_name );
   BOOST_REQUIRE_EQUAL( success(), stake( "bob111111111", core_sym::from_string("50.00000000"), core_sym::from_string("50.00000000") ) );
   REQUIRE_MATCHING_OBJECT( voter( "bob111111111", core_sym::from_string("100.00000000") ), get_voter_info( "bob111111111" ) );
...
BOOST_FIXTURE_TEST_CASE( producer_keep_votes, eosio_system_tester, * boost::unit_test::tolerance(1e+5) ) try {
   ...
   auto prod = get_producer_info( "alice1111111" );
   BOOST_TEST_REQUIRE( stake2votes(core_sym::from_string("13.57910000")) == prod["total_votes"].as_double() );
...
BOOST_FIXTURE_TEST_CASE( vote_for_two_producers, eosio_system_tester, * boost::unit_test::tolerance(1e+5) ) try {
   ...
   issue_and_transfer( "carol1111111", core_sym::from_string("1000.00000000"),  config::system_account_name );
   BOOST_REQUIRE_EQUAL( success(), stake( "carol1111111", core_sym::from_string("15.00050000"), core_sym::from_string("5.00000000") ) );
   ...
   BOOST_TEST_REQUIRE( stake2votes(core_sym::from_string("20.00050000")) == alice_info["total_votes"].as_double() );
   auto bob_info = get_producer_info( "bob111111111" );
   BOOST_TEST_REQUIRE( stake2votes(core_sym::from_string("20.00050000")) == bob_info["total_votes"].as_double() );
   ...
   issue_and_transfer( "alice1111111", core_sym::from_string("2.00000000"),  config::system_account_name );
   BOOST_REQUIRE_EQUAL( success(), stake( "alice1111111", core_sym::from_string("1.00000000"), core_sym::from_string("1.00000000") ) );
   BOOST_REQUIRE_EQUAL( success(), vote(N(alice1111111), { N(alice1111111), N(bob111111111) } ) );

   alice_info = get_producer_info( "alice1111111" );
   BOOST_TEST_REQUIRE( stake2votes(core_sym::from_string("22.00050000")) == alice_info["total_votes"].as_double() );

   bob_info = get_producer_info( "bob111111111" );
   BOOST_TEST_REQUIRE( stake2votes(core_sym::from_string("2.00000000")) == bob_info["total_votes"].as_double() );
...
BOOST_FIXTURE_TEST_CASE( proxy_register_unregister_keeps_stake, eosio_system_tester ) try {
   ...
   issue_and_transfer( "bob111111111", core_sym::from_string("1000.00000000"),  config::system_account_name );
   BOOST_REQUIRE_EQUAL( success(), stake( "bob111111111", core_sym::from_string("200.00020000"), core_sym::from_string("100.00010000") ) );
   BOOST_REQUIRE_EQUAL( success(), push_action( N(bob111111111), N(regproxy), mvo()
                                               ("proxy",  "bob111111111")
                                               ("isproxy", true)
                        )
   );
   REQUIRE_MATCHING_OBJECT( proxy( N(bob111111111) )( "staked", 30000030000 ), get_voter_info( "bob111111111" ) );
   ...
   REQUIRE_MATCHING_OBJECT( voter( "bob111111111", core_sym::from_string("300.00030000") ), get_voter_info( "bob111111111" ) );
   ...
   issue_and_transfer( "carol1111111", core_sym::from_string("1000.00000000"),  config::system_account_name );
   BOOST_REQUIRE_EQUAL( success(), stake( "carol1111111", core_sym::from_string("246.00020000"), core_sym::from_string("531.00010000") ) );
   //check that both proxy flag and stake a correct
   REQUIRE_MATCHING_OBJECT( proxy( N(carol1111111) )( "staked", 77700030000 ), get_voter_info( "carol1111111" ) );
   ...
   REQUIRE_MATCHING_OBJECT( voter( "carol1111111", core_sym::from_string("777.00030000") ), get_voter_info( "carol1111111" ) );
...
BOOST_FIXTURE_TEST_CASE( proxy_stake_unstake_keeps_proxy_flag, eosio_system_tester ) try {
   ...
   //stake
   BOOST_REQUIRE_EQUAL( success(), stake( "alice1111111", core_sym::from_string("100.00000000"), core_sym::from_string("50.00000000") ) );
   //check that account is still a proxy
   REQUIRE_MATCHING_OBJECT( proxy( N(alice1111111) )( "staked", 15000000000 ), get_voter_info( "alice1111111" ) );

   //stake more
   BOOST_REQUIRE_EQUAL( success(), stake( "alice1111111", core_sym::from_string("30.00000000"), core_sym::from_string("20.00000000") ) );
   //check that account is still a proxy
   REQUIRE_MATCHING_OBJECT( proxy( N(alice1111111) )("staked", 20000000000 ), get_voter_info( "alice1111111" ) );

   //unstake more
   BOOST_REQUIRE_EQUAL( success(), unstake( "alice1111111", core_sym::from_string("65.00000000"), core_sym::from_string("35.00000000") ) );
   REQUIRE_MATCHING_OBJECT( proxy( N(alice1111111) )("staked", 10000000000 ), get_voter_info( "alice1111111" ) );
...
BOOST_FIXTURE_TEST_CASE( proxy_actions_affect_producers, eosio_system_tester, * boost::unit_test::tolerance(1e+5) ) try {
   ...
   issue_and_transfer( "bob111111111", core_sym::from_string("1000.00000000"),  config::system_account_name );
   BOOST_REQUIRE_EQUAL( success(), stake( "bob111111111", core_sym::from_string("100.00020000"), core_sym::from_string("50.00010000") ) );
   BOOST_REQUIRE_EQUAL( success(), vote(N(bob111111111), vector<account_name>(), N(alice1111111) ) );
   REQUIRE_MATCHING_OBJECT( proxy( N(alice1111111) )( "proxied_vote_weight", stake2votes(core_sym::from_string("150.00030000")) ), get_voter_info( "alice1111111" ) );

   //vote for producers
   BOOST_REQUIRE_EQUAL( success(), vote(N(alice1111111), { N(defproducer1), N(defproducer2) } ) );
   BOOST_TEST_REQUIRE( stake2votes(core_sym::from_string("150.00030000")) == get_producer_info( "defproducer1" )["total_votes"].as_double() );
   BOOST_TEST_REQUIRE( stake2votes(core_sym::from_string("150.00030000")) == get_producer_info( "defproducer2" )["total_votes"].as_double() );
   ...
   BOOST_TEST_REQUIRE( stake2votes(core_sym::from_string("150.00030000")) == get_producer_info( "defproducer3" )["total_votes"].as_double() );
   ...
   //REQUIRE_MATCHING_OBJECT( voter( "alice1111111" )( "proxied_vote_weight", stake2votes(core_sym::from_string("150.00030000")) ), get_voter_info( "alice1111111" ) );
   ...
   issue_and_transfer( "alice1111111", core_sym::from_string("1000.00000000"),  config::system_account_name );
   BOOST_REQUIRE_EQUAL( success(), stake( "alice1111111", core_sym::from_string("30.00010000"), core_sym::from_string("20.00010000") ) );
   BOOST_REQUIRE_EQUAL( stake2votes(core_sym::from_string("200.00050000")), get_producer_info( "defproducer1" )["total_votes"].as_double() );
   BOOST_REQUIRE_EQUAL( 0, get_producer_info( "defproducer2" )["total_votes"].as_double() );
   BOOST_REQUIRE_EQUAL( stake2votes(core_sym::from_string("200.00050000")), get_producer_info( "defproducer3" )["total_votes"].as_double() );

   //stake decrease by proxy itself affects producers
   BOOST_REQUIRE_EQUAL( success(), unstake( "alice1111111", core_sym::from_string("10.00010000"), core_sym::from_string("10.00010000") ) );
   BOOST_TEST_REQUIRE( stake2votes(core_sym::from_string("180.00030000")) == get_producer_info( "defproducer1" )["total_votes"].as_double() );
   BOOST_REQUIRE_EQUAL( 0, get_producer_info( "defproducer2" )["total_votes"].as_double() );
   BOOST_TEST_REQUIRE( stake2votes(core_sym::from_string("180.00030000")) == get_producer_info( "defproducer3" )["total_votes"].as_double() );
...
BOOST_FIXTURE_TEST_CASE(producer_pay, eosio_system_tester, * boost::unit_test::tolerance(1e-10)) try {
   ...
   const asset large_asset = core_sym::from_string("80.00000000");
   create_account_with_resources( N(defproducera), config::system_account_name, core_sym::from_string("1.00000000"), false, large_asset, large_asset );
   create_account_with_resources( N(defproducerb), config::system_account_name, core_sym::from_string("1.00000000"), false, large_asset, large_asset );
   create_account_with_resources( N(defproducerc), config::system_account_name, core_sym::from_string("1.00000000"), false, large_asset, large_asset );

   create_account_with_resources( N(producvotera), config::system_account_name, core_sym::from_string("1.00000000"), false, large_asset, large_asset );
   create_account_with_resources( N(producvoterb), config::system_account_name, core_sym::from_string("1.00000000"), false, large_asset, large_asset );
   ...
   transfer( config::system_account_name, "producvotera", core_sym::from_string("40000.00000000"), config::system_account_name);
   BOOST_REQUIRE_EQUAL(success(), stake("producvotera", core_sym::from_string("10000.00000000"), core_sym::from_string("10000.00000000")));
...
BOOST_AUTO_TEST_CASE(extreme_inflation) try {
   ...
   t.issue( asset(100000, core_symbol) );
   t.deploy_contract();
   t.produce_block();
   t.create_account_with_resources(N(defproducera), config::system_account_name, core_sym::from_string("1.00000000"), false);
   BOOST_REQUIRE_EQUAL(t.success(), t.regproducer(N(defproducera)));
   t.transfer( config::system_account_name, "defproducera", core_sym::from_string("20000.00000000"), config::system_account_name);
   BOOST_REQUIRE_EQUAL(t.success(), t.stake("defproducera", core_sym::from_string("10000.00000000"), core_sym::from_string("10000.00000000")));
...
BOOST_FIXTURE_TEST_CASE(multiple_producer_pay, eosio_system_tester, * boost::unit_test::tolerance(1e-10)) try {
   ...
   const asset net = core_sym::from_string("80.00000000");
   const asset cpu = core_sym::from_string("80.00000000");
   const std::vector<account_name> voters = { N(producvotera), N(producvoterb), N(producvoterc), N(producvoterd) };
   for (const auto& v: voters) {
      create_account_with_resources( v, config::system_account_name, core_sym::from_string("1.00000000"), false, net, cpu );
      transfer( config::system_account_name, v, core_sym::from_string("10000.00000000"), config::system_account_name );
      BOOST_REQUIRE_EQUAL(success(), stake(v, core_sym::from_string("3000.00000000"), core_sym::from_string("3000.00000000")) );
   }
...
BOOST_FIXTURE_TEST_CASE(votepay_share_invariant, eosio_system_tester, * boost::unit_test::tolerance(1e-10)) try {
   ...
   for (const auto& a: accounts) {
      create_account_with_resources( a, config::system_account_name, core_sym::from_string("1.00000000"), false, net, cpu );
      transfer( config::system_account_name, a, core_sym::from_string("1000.00000000"), config::system_account_name );
   }
   ...
   BOOST_REQUIRE_EQUAL( success(), stake( vota, core_sym::from_string("100.00000000"), core_sym::from_string("100.00000000") ) );
   BOOST_REQUIRE_EQUAL( success(), stake( votb, core_sym::from_string("100.00000000"), core_sym::from_string("100.00000000") ) );
...
BOOST_FIXTURE_TEST_CASE(votepay_share_proxy, eosio_system_tester, * boost::unit_test::tolerance(1e-5)) try {
   ...
   BOOST_REQUIRE_EQUAL( success(), stake( bob, core_sym::from_string("100.00020000"), core_sym::from_string("50.00010000") ) );
   BOOST_REQUIRE_EQUAL( success(), stake( alice, core_sym::from_string("150.00000000"), core_sym::from_string("150.00000000") ) );
   BOOST_REQUIRE_EQUAL( success(), vote( bob, { }, alice ) );
   BOOST_TEST_REQUIRE( stake2votes(core_sym::from_string("150.00030000")) == get_voter_info(alice)["proxied_vote_weight"].as_double() );
   ...
   BOOST_TEST_REQUIRE( stake2votes(core_sym::from_string("450.00030000")) == total_votes );
   ...
   BOOST_TEST_REQUIRE( stake2votes(core_sym::from_string("450.00030000")) == get_producer_info(carol)["total_votes"].as_double() );
   ...
   // bob unstakes
   BOOST_REQUIRE_EQUAL( success(), unstake( bob, core_sym::from_string("10.00020000"), core_sym::from_string("10.00010000") ) );
   BOOST_TEST_REQUIRE( stake2votes(core_sym::from_string("430.00000000")), get_producer_info(carol)["total_votes"].as_double() );
...
BOOST_FIXTURE_TEST_CASE(votepay_share_update_order, eosio_system_tester, * boost::unit_test::tolerance(1e-10)) try {
   ...
   BOOST_REQUIRE_EQUAL( success(), stake( alice, core_sym::from_string("100.00000000"), core_sym::from_string("100.00000000") ) );
   BOOST_REQUIRE_EQUAL( success(), stake( bob,   core_sym::from_string("100.00000000"), core_sym::from_string("100.00000000") ) );
...
BOOST_FIXTURE_TEST_CASE(votepay_transition, eosio_system_tester, * boost::unit_test::tolerance(1e-10)) try {
   ...
   create_account_with_resources( N(defproducer1), config::system_account_name, core_sym::from_string("1.00000000"), false, net, cpu );
...
BOOST_AUTO_TEST_CASE(votepay_transition2, * boost::unit_test::tolerance(1e-10)) try {
   ...
   const asset net = old_core_from_string("80.00000000");
   const asset cpu = old_core_from_string("80.00000000");
   const std::vector<account_name> voters = { N(producvotera), N(producvoterb), N(producvoterc), N(producvoterd) };
   for (const auto& v: voters) {
      t.create_account_with_resources( v, config::system_account_name, old_core_from_string("1.00000000"), false, net, cpu );
      t.transfer( config::system_account_name, v, old_core_from_string("10000.00000000"), config::system_account_name );
      BOOST_REQUIRE_EQUAL(t.success(), t.stake(v, old_core_from_string("3000.00000000"), old_core_from_string("3000.00000000")) );
   }
   ...
   std::vector<account_name> producer_names;
   {
     ...
     t.setup_producer_accounts( producer_names, old_core_from_string("1.00000000"),
                     old_core_from_string("80.00000000"), old_core_from_string("80.00000000") );
      for (const auto& p: producer_names) {
...
BOOST_FIXTURE_TEST_CASE(producer_onblock_check, eosio_system_tester) try {
   ...
   create_account_with_resources( N(producvotera), config::system_account_name, core_sym::from_string("1.00000000"), false, large_asset, large_asset );
   create_account_with_resources( N(producvoterb), config::system_account_name, core_sym::from_string("1.00000000"), false, large_asset, large_asset );
   create_account_with_resources( N(producvoterc), config::system_account_name, core_sym::from_string("1.00000000"), false, large_asset, large_asset );
   ...
   transfer(config::system_account_name, "producvotera", core_sym::from_string("20000.00000000"), config::system_account_name);
   BOOST_REQUIRE_EQUAL(success(), stake("producvotera", core_sym::from_string("7000.00000000"), core_sym::from_string("7000.00000000") ));
   BOOST_REQUIRE_EQUAL(success(), vote( N(producvotera), vector<account_name>(producer_names.begin(), producer_names.begin()+10)));
   BOOST_CHECK_EQUAL( wasm_assert_msg( "cannot undelegate bandwidth until the chain is activated (at least 15% of all tokens participate in voting)" ),
                      unstake( "producvotera", core_sym::from_string("50.00000000"), core_sym::from_string("50.00000000") ) );
   ...
   transfer(config::system_account_name, "producvoterb", core_sym::from_string("10000.00000000"), config::system_account_name);
   BOOST_REQUIRE_EQUAL(success(), stake("producvoterb", core_sym::from_string("400.00000000"), core_sym::from_string("400.00000000")));
   transfer(config::system_account_name, "producvoterc", core_sym::from_string("10000.00000000"), config::system_account_name);
   BOOST_REQUIRE_EQUAL(success(), stake("producvoterc", core_sym::from_string("200.00000000"), core_sym::from_string("200.00000000")));
   ...
   BOOST_CHECK_EQUAL( success(), unstake( "producvotera", core_sym::from_string("50.00000000"), core_sym::from_string("50.00000000") ) );
...
BOOST_FIXTURE_TEST_CASE( voters_actions_affect_proxy_and_producers, eosio_system_tester, * boost::unit_test::tolerance(1e+6) ) try {
   ...
   BOOST_REQUIRE_EQUAL( success(), stake( "alice1111111", core_sym::from_string("30.00010000"), core_sym::from_string("20.00010000") ) );
   BOOST_REQUIRE_EQUAL( success(), vote( N(alice1111111), { N(defproducer1), N(defproducer2) } ) );
   BOOST_TEST_REQUIRE( stake2votes(core_sym::from_string("50.00020000")) == get_producer_info( "defproducer1" )["total_votes"].as_double() );
   BOOST_TEST_REQUIRE( stake2votes(core_sym::from_string("50.00020000")) == get_producer_info( "defproducer2" )["total_votes"].as_double() );
   BOOST_REQUIRE_EQUAL( 0, get_producer_info( "defproducer3" )["total_votes"].as_double() );
   ...
   BOOST_TEST_REQUIRE( stake2votes(core_sym::from_string("150.00030000")) == get_voter_info( "alice1111111" )["proxied_vote_weight"].as_double() );
   BOOST_TEST_REQUIRE( stake2votes(core_sym::from_string("200.00050000")) == get_producer_info( "defproducer1" )["total_votes"].as_double() );
   BOOST_TEST_REQUIRE( stake2votes(core_sym::from_string("200.00050000")) == get_producer_info( "defproducer2" )["total_votes"].as_double() );
   BOOST_REQUIRE_EQUAL( 0, get_producer_info( "defproducer3" )["total_votes"].as_double() );

   //carol1111111 chooses alice1111111 as a proxy
   issue_and_transfer( "carol1111111", core_sym::from_string("1000.00000000"),  config::system_account_name );
   BOOST_REQUIRE_EQUAL( success(), stake( "carol1111111", core_sym::from_string("30.00010000"), core_sym::from_string("20.00010000") ) );
   BOOST_REQUIRE_EQUAL( success(), vote( N(carol1111111), vector<account_name>(), "alice1111111" ) );
   BOOST_TEST_REQUIRE( stake2votes(core_sym::from_string("200.00050000")) == get_voter_info( "alice1111111" )["proxied_vote_weight"].as_double() );
   BOOST_TEST_REQUIRE( stake2votes(core_sym::from_string("250.00070000")) == get_producer_info( "defproducer1" )["total_votes"].as_double() );
   BOOST_TEST_REQUIRE( stake2votes(core_sym::from_string("250.00070000")) == get_producer_info( "defproducer2" )["total_votes"].as_double() );
   BOOST_REQUIRE_EQUAL( 0, get_producer_info( "defproducer3" )["total_votes"].as_double() );

   //proxied voter carol1111111 increases stake
   BOOST_REQUIRE_EQUAL( success(), stake( "carol1111111", core_sym::from_string("50.00000000"), core_sym::from_string("70.00000000") ) );
   BOOST_TEST_REQUIRE( stake2votes(core_sym::from_string("320.00050000")) == get_voter_info( "alice1111111" )["proxied_vote_weight"].as_double() );
   BOOST_TEST_REQUIRE( stake2votes(core_sym::from_string("370.00070000")) == get_producer_info( "defproducer1" )["total_votes"].as_double() );
   BOOST_TEST_REQUIRE( stake2votes(core_sym::from_string("370.00070000")) == get_producer_info( "defproducer2" )["total_votes"].as_double() );
   BOOST_REQUIRE_EQUAL( 0, get_producer_info( "defproducer3" )["total_votes"].as_double() );

   //proxied voter bob111111111 decreases stake
   BOOST_REQUIRE_EQUAL( success(), unstake( "bob111111111", core_sym::from_string("50.00010000"), core_sym::from_string("50.00010000") ) );
   BOOST_TEST_REQUIRE( stake2votes(core_sym::from_string("220.00030000")) == get_voter_info( "alice1111111" )["proxied_vote_weight"].as_double() );
   BOOST_TEST_REQUIRE( stake2votes(core_sym::from_string("270.00050000")) == get_producer_info( "defproducer1" )["total_votes"].as_double() );
   BOOST_TEST_REQUIRE( stake2votes(core_sym::from_string("270.00050000")) == get_producer_info( "defproducer2" )["total_votes"].as_double() );
   BOOST_REQUIRE_EQUAL( 0, get_producer_info( "defproducer3" )["total_votes"].as_double() );
   ...
   BOOST_TEST_REQUIRE( stake2votes(core_sym::from_string("50.00010000")), get_voter_info( "alice1111111" )["proxied_vote_weight"].as_double() );
   BOOST_TEST_REQUIRE( stake2votes(core_sym::from_string("170.00020000")), get_voter_info( "donald111111" )["proxied_vote_weight"].as_double() );
   BOOST_TEST_REQUIRE( stake2votes(core_sym::from_string("100.00030000")), get_producer_info( "defproducer1" )["total_votes"].as_double() );
   BOOST_TEST_REQUIRE( stake2votes(core_sym::from_string("100.00030000")), get_producer_info( "defproducer2" )["total_votes"].as_double() );
   BOOST_REQUIRE_EQUAL( 0, get_producer_info( "defproducer3" )["total_votes"].as_double() );
...
BOOST_FIXTURE_TEST_CASE( double_register_unregister_proxy_keeps_votes, eosio_system_tester ) try {
   ...
   issue_and_transfer( "alice1111111", core_sym::from_string("1000.00000000"),  config::system_account_name );
   BOOST_REQUIRE_EQUAL( success(), stake( "alice1111111", core_sym::from_string("5.00000000"), core_sym::from_string("5.00000000") ) );
   edump((get_voter_info("alice1111111")));
   REQUIRE_MATCHING_OBJECT( proxy( N(alice1111111) )( "staked", 1000000000 ), get_voter_info( "alice1111111" ) );

   //bob111111111 stakes and selects alice1111111 as a proxy
   issue_and_transfer( "bob111111111", core_sym::from_string("1000.00000000"),  config::system_account_name );
   BOOST_REQUIRE_EQUAL( success(), stake( "bob111111111", core_sym::from_string("100.00020000"), core_sym::from_string("50.00010000") ) );
   BOOST_REQUIRE_EQUAL( success(), vote( N(bob111111111), vector<account_name>(), "alice1111111" ) );
   REQUIRE_MATCHING_OBJECT( proxy( N(alice1111111) )( "proxied_vote_weight", stake2votes( core_sym::from_string("150.00030000") ))( "staked", 1000000000 ), get_voter_info( "alice1111111" ) );
   ...
   REQUIRE_MATCHING_OBJECT( voter( "alice1111111" )( "proxied_vote_weight", stake2votes(core_sym::from_string("150.00030000")) )( "staked", 1000000000 ), get_voter_info( "alice1111111" ) );
...
BOOST_FIXTURE_TEST_CASE( proxy_cannot_use_another_proxy, eosio_system_tester ) try {
   ...
   issue_and_transfer( "carol1111111", core_sym::from_string("1000.00000000"),  config::system_account_name );
   BOOST_REQUIRE_EQUAL( success(), stake( "carol1111111", core_sym::from_string("100.00020000"), core_sym::from_string("50.00010000") ) );
   BOOST_REQUIRE_EQUAL( success(), vote( N(carol1111111), vector<account_name>(), "alice1111111" ) );
...
BOOST_FIXTURE_TEST_CASE( elect_producers /*_and_parameters*/, eosio_system_tester ) try {
   ...
   transfer( "eosio", "alice1111111", core_sym::from_string("60000.00000000"), "eosio" );
   BOOST_REQUIRE_EQUAL( success(), stake( "alice1111111", "alice1111111", core_sym::from_string("30000.00000000"), core_sym::from_string("30000.00000000") ) );
   //vote for producers
   ...
   // elect 2 producers
   issue_and_transfer( "bob111111111", core_sym::from_string("80000.00000000"),  config::system_account_name );
   ilog("stake");
   BOOST_REQUIRE_EQUAL( success(), stake( "bob111111111", core_sym::from_string("40000.00000000"), core_sym::from_string("40000.00000000") ) );
   ilog("start vote");
   BOOST_REQUIRE_EQUAL( success(), vote( N(bob111111111), { N(defproducer2) } ) );
   ilog(".");
   produce_blocks(250);
...
BOOST_FIXTURE_TEST_CASE( buyname, eosio_system_tester ) try {
   ...
   transfer( config::system_account_name, "dan", core_sym::from_string( "10000.00000000" ) );
   transfer( config::system_account_name, "sam", core_sym::from_string( "10000.00000000" ) );
   stake_with_transfer( config::system_account_name, N(sam), core_sym::from_string( "8000.00000000" ), core_sym::from_string( "8000.00000000" ) );
   stake_with_transfer( config::system_account_name, N(dan), core_sym::from_string( "8000.00000000" ), core_sym::from_string( "8000.00000000" ) );
   ...
   bidname( "dan", "nofail", core_sym::from_string( "1.00000000" ) );
   BOOST_REQUIRE_EQUAL( "assertion failure with message: must increase bid by 10%", bidname( "sam", "nofail", core_sym::from_string( "1.00000000" ) )); // didn't increase bid by 10%
   BOOST_REQUIRE_EQUAL( success(), bidname( "sam", "nofail", core_sym::from_string( "2.00000000" ) )); // didn't increase bid by 10%
   produce_block( fc::days(1) );
   produce_block();
   ...
   transfer( "eosio", "nofail", core_sym::from_string( "1000.00000000" ) );
...
BOOST_FIXTURE_TEST_CASE( multiple_namebids, eosio_system_tester ) try {
   ...
   for ( const auto& a: accounts ) {
      transfer( config::system_account_name, a, core_sym::from_string( "10000.00000000" ) );
      BOOST_REQUIRE_EQUAL( core_sym::from_string( "10000.00000000" ), get_balance(a) );
   }
   ...
   stake_with_transfer( config::system_account_name, N(bob),  core_sym::from_string( "3500.00000000" ), core_sym::from_string( "3500.00000000" ) );
   stake_with_transfer( config::system_account_name, N(carl), core_sym::from_string( "3500.00000000" ), core_sym::from_string( "3500.00000000" ) );
   BOOST_REQUIRE_EQUAL( success(), vote( N(bob), { N(producer) } ) );
   BOOST_REQUIRE_EQUAL( success(), vote( N(carl), { N(producer) } ) );

   // start bids
   bidname( "bob",  "prefa", core_sym::from_string("1.00030000") );
   BOOST_REQUIRE_EQUAL( core_sym::from_string( "9998.99970000" ), get_balance("bob") );
   bidname( "bob",  "prefb", core_sym::from_string("1.00000000") );
   bidname( "bob",  "prefc", core_sym::from_string("1.00000000") );
   BOOST_REQUIRE_EQUAL( core_sym::from_string( "9996.99970000" ), get_balance("bob") );

   bidname( "carl", "prefd", core_sym::from_string("1.00000000") );
   bidname( "carl", "prefe", core_sym::from_string("1.00000000") );
   BOOST_REQUIRE_EQUAL( core_sym::from_string( "9998.00000000" ), get_balance("carl") );

   BOOST_REQUIRE_EQUAL( error("assertion failure with message: account is already highest bidder"),
                        bidname( "bob", "prefb", core_sym::from_string("1.10010000") ) );
   BOOST_REQUIRE_EQUAL( error("assertion failure with message: must increase bid by 10%"),
                        bidname( "alice", "prefb", core_sym::from_string("1.09990000") ) );
   BOOST_REQUIRE_EQUAL( core_sym::from_string( "9996.99970000" ), get_balance("bob") );
   BOOST_REQUIRE_EQUAL( core_sym::from_string( "10000.00000000" ), get_balance("alice") );

   // alice outbids bob on prefb
   {
      const asset initial_names_balance = get_balance(N(eosio.names));
      BOOST_REQUIRE_EQUAL( success(),
                           bidname( "alice", "prefb", core_sym::from_string("1.10010000") ) );
      BOOST_REQUIRE_EQUAL( core_sym::from_string( "9997.99970000" ), get_balance("bob") );
      BOOST_REQUIRE_EQUAL( core_sym::from_string( "9998.89990000" ), get_balance("alice") );
      BOOST_REQUIRE_EQUAL( initial_names_balance + core_sym::from_string("0.10010000"), get_balance(N(eosio.names)) );
   }

   // david outbids carl on prefd
   {
      BOOST_REQUIRE_EQUAL( core_sym::from_string( "9998.00000000" ), get_balance("carl") );
      BOOST_REQUIRE_EQUAL( core_sym::from_string( "10000.00000000" ), get_balance("david") );
      BOOST_REQUIRE_EQUAL( success(),
                           bidname( "david", "prefd", core_sym::from_string("1.99000000") ) );
      BOOST_REQUIRE_EQUAL( core_sym::from_string( "9999.00000000" ), get_balance("carl") );
      BOOST_REQUIRE_EQUAL( core_sym::from_string( "9998.01000000" ), get_balance("david") );
   }

   // eve outbids carl on prefe
   {
      BOOST_REQUIRE_EQUAL( success(),
                           bidname( "eve", "prefe", core_sym::from_string("1.72000000") ) );
   }
   ...
   stake_with_transfer( config::system_account_name, N(alice), core_sym::from_string( "15000.00000000" ), core_sym::from_string( "15000.00000000" ) );
   ...
   BOOST_REQUIRE_EQUAL( success(),
                        bidname( "eve",  "prefb", core_sym::from_string("2.18800000") ) );
   ...
   BOOST_REQUIRE_EQUAL( success(),
                        bidname( "carl", "prefe", core_sym::from_string("2.09800000") ) );
   produce_block( fc::hours(2) );
   produce_blocks(2);
   ...
   transfer( config::system_account_name, N(prefe), core_sym::from_string("10000.00000000") );
...
BOOST_FIXTURE_TEST_CASE( namebid_pending_winner, eosio_system_tester ) try {
   ...
   transfer( config::system_account_name, N(alice1111111), core_sym::from_string("10000.00000000") );
   transfer( config::system_account_name, N(bob111111111), core_sym::from_string("10000.00000000") );

   BOOST_REQUIRE_EQUAL( success(), bidname( "alice1111111", "prefa", core_sym::from_string( "50.00000000" ) ));
   BOOST_REQUIRE_EQUAL( success(), bidname( "bob111111111", "prefb", core_sym::from_string( "30.00000000" ) ));
...
BOOST_FIXTURE_TEST_CASE( vote_producers_in_and_out, eosio_system_tester ) try {
   ...
   for (const auto& v: voters) {
      transfer( config::system_account_name, v, core_sym::from_string("20000.00000000"), config::system_account_name );
      BOOST_REQUIRE_EQUAL(success(), stake(v, core_sym::from_string("3000.00000000"), core_sym::from_string("3000.00000000")) );
   }
...

   {
      ...
      BOOST_REQUIRE_EQUAL(success(), stake("producvoterd", core_sym::from_string("4000.00000000"), core_sym::from_string("4000.00000000")));
...
BOOST_FIXTURE_TEST_CASE( setram_effect, eosio_system_tester ) try {
   ...
   const asset net = core_sym::from_string("8.00000000");
   const asset cpu = core_sym::from_string("8.00000000");
...

   {
      ...
      transfer( config::system_account_name, name_b, core_sym::from_string("1000.00000000") );
      BOOST_REQUIRE_EQUAL( core_sym::from_string("1000.00000000"), get_balance(name_b) );
      const uint64_t init_bytes_b = get_total_stake(name_b)["ram_bytes"].as_uint64();
      // name_b buys ram at current price
      BOOST_REQUIRE_EQUAL( success(), buyram( name_b, name_b, core_sym::from_string("300.00000000") ) );
      BOOST_REQUIRE_EQUAL( core_sym::from_string("700.00000000"), get_balance(name_b) );
      ...
      BOOST_REQUIRE( core_sym::from_string("900.00000000") < get_balance(name_b) );
      BOOST_REQUIRE( core_sym::from_string("950.00000000") > get_balance(name_b) );
   }
...
BOOST_FIXTURE_TEST_CASE( ram_inflation, eosio_system_tester ) try {
   ...
   produce_blocks(20);
   BOOST_REQUIRE_EQUAL( init_max_ram_size, get_global_state()["max_ram_size"].as_uint64() );
   transfer( config::system_account_name, "alice1111111", core_sym::from_string("1000.00000000"), config::system_account_name );
   BOOST_REQUIRE_EQUAL( success(), buyram( "alice1111111", "alice1111111", core_sym::from_string("100.00000000") ) );
   produce_blocks(3);
   BOOST_REQUIRE_EQUAL( init_max_ram_size, get_global_state()["max_ram_size"].as_uint64() );
   uint16_t rate = 1000;
...
BOOST_FIXTURE_TEST_CASE( ram_gift, eosio_system_tester ) try {
   ...
   */
   ...
   BOOST_REQUIRE_EQUAL( success(), unstake( "eosio", "alice1111111", core_sym::from_string("20.00000000"), core_sym::from_string("10.00000000") ) );
...
BOOST_FIXTURE_TEST_CASE( rex_auth, eosio_system_tester ) try {
   ...
   const asset one_eos      = core_sym::from_string("1.00000000");
   const asset one_rex      = asset::from_string("1.00000000 REX");
...
BOOST_FIXTURE_TEST_CASE( buy_sell_rex, eosio_system_tester ) try {
   ...
   BOOST_REQUIRE_EQUAL( success(),                           buyrex( alice, core_sym::from_string("17.00000000") ) );
   BOOST_REQUIRE_EQUAL( core_sym::from_string("30.00000000"),    get_rex_vote_stake( alice ) );
   BOOST_REQUIRE_EQUAL( core_sym::from_string("970.00000000"),   get_rex_fund(alice) );
   BOOST_REQUIRE_EQUAL( get_rex_balance(alice).get_amount(), ratio * asset::from_string("30.00000000 REX").get_amount() );
   auto rex_pool = get_rex_pool();
   BOOST_REQUIRE_EQUAL( core_sym::from_string("30.00000000"),  rex_pool["total_lendable"].as<asset>() );
   BOOST_REQUIRE_EQUAL( core_sym::from_string("30.00000000"),  rex_pool["total_unlent"].as<asset>() );
...
BOOST_FIXTURE_TEST_CASE( buy_sell_small_rex, eosio_system_tester ) try {
   ...
   const int64_t ratio        = rex_ratio;
   const asset   init_balance = core_sym::from_string("50000.00000000");
   ...
   const asset payment = core_sym::from_string("40000.00000000");
   ...
   BOOST_REQUIRE_EQUAL( wasm_assert_msg("proceeds are negligible"), sellrex( alice, asset::from_string("0.00000001 REX") ) );
   //BOOST_REQUIRE_EQUAL( wasm_assert_msg("proceeds are negligible"), sellrex( alice, asset::from_string("0.00009999 REX") ) );
   BOOST_REQUIRE_EQUAL( core_sym::from_string("0.10000000"),            get_sellrex_result( alice, asset::from_string("1.00000000 REX") ) );
   BOOST_REQUIRE_EQUAL( core_sym::from_string("0.19999000"),            get_sellrex_result( alice, asset::from_string("1.99990000 REX") ) );
   BOOST_REQUIRE_EQUAL( core_sym::from_string("0.20000000"),            get_sellrex_result( alice, asset::from_string("2.00000000 REX") ) );
   BOOST_REQUIRE_EQUAL( core_sym::from_string("0.29999000"),            get_sellrex_result( alice, asset::from_string("2.99990000 REX") ) );
   BOOST_REQUIRE_EQUAL( get_rex_vote_stake( alice ),                init_rex_stake - core_sym::from_string("0.79998000") );

   BOOST_REQUIRE_EQUAL( success(),                                  rentcpu( carol, bob, core_sym::from_string("10.00000000") ) );
   BOOST_REQUIRE_EQUAL( success(),                                  sellrex( alice, asset::from_string("1.00000000 REX") ) );
   BOOST_REQUIRE_EQUAL( wasm_assert_msg("proceeds are negligible"), sellrex( alice, asset::from_string("0.00000009 REX") ) );
...
BOOST_FIXTURE_TEST_CASE( unstake_buy_rex, eosio_system_tester, * boost::unit_test::tolerance(1e-10) ) try {
   ...
   const int64_t ratio        = rex_ratio;
   const asset   zero_asset   = core_sym::from_string("0.00000000");
   const asset   neg_asset    = core_sym::from_string("-0.00010000");
   const asset   one_token    = core_sym::from_string("1.00000000");
   const asset   init_balance = core_sym::from_string("10000.00000000");
   const asset   init_net     = core_sym::from_string("70.00000000");
   const asset   init_cpu     = core_sym::from_string("90.00000000");
   const std::vector<account_name> accounts = { N(aliceaccount), N(bobbyaccount), N(carolaccount), N(emilyaccount), N(frankaccount) };
   account_name alice = accounts[0], bob = accounts[1], carol = accounts[2], emily = accounts[3], frank = accounts[4];
   ...
   {
      const asset net_stake = core_sym::from_string("25.50000000");
      const asset cpu_stake = core_sym::from_string("12.40000000");
   ...
   {
      const asset net_stake = core_sym::from_string("200.50000000");
      const asset cpu_stake = core_sym::from_string("120.00000000");
      const asset tot_stake = net_stake + cpu_stake;
   ...
   {
      const asset net_stake = core_sym::from_string("130.50000000");
      const asset cpu_stake = core_sym::from_string("220.08000000");
      const asset tot_stake = net_stake + cpu_stake;
      ...
      BOOST_REQUIRE_EQUAL( wasm_assert_msg("insufficient available rex"),
                           sellrex( frank, asset::from_string("1.00000000 REX") ) );
      produce_block( fc::days(5) );
      BOOST_REQUIRE_EQUAL( success(),                               sellrex( frank, asset::from_string("1.00000000 REX") ) );
   }
...
BOOST_FIXTURE_TEST_CASE( buy_rent_rex, eosio_system_tester ) try {
   ...
   const int64_t ratio        = rex_ratio;
   const asset   init_balance = core_sym::from_string("60000.00000000");
   const asset   init_net     = core_sym::from_string("70.00000000");
   const asset   init_cpu     = core_sym::from_string("90.00000000");
   const std::vector<account_name> accounts = { N(aliceaccount), N(bobbyaccount), N(carolaccount), N(emilyaccount), N(frankaccount) };
   account_name alice = accounts[0], bob = accounts[1], carol = accounts[2], emily = accounts[3], frank = accounts[4];
   ...
   BOOST_REQUIRE_EQUAL( wasm_assert_msg("rex system not initialized yet"), rentcpu( bob, carol, core_sym::from_string("5.00000000") ) );
   // alice lends rex
   BOOST_REQUIRE_EQUAL( success(), buyrex( alice, core_sym::from_string("50265.00000000") ) );
   BOOST_REQUIRE_EQUAL( init_balance - core_sym::from_string("50265.00000000"), get_rex_fund(alice) );
   auto rex_pool = get_rex_pool();
...

   {
      ...
      const asset fee = core_sym::from_string("17.00000000");
...

   {
      ...
      BOOST_REQUIRE_EQUAL( 0,         get_rex_balance(alice).get_amount() );
      BOOST_REQUIRE_EQUAL( success(), buyrex( alice, core_sym::from_string("20050.00000000") ) );
      rex_pool = get_rex_pool();
      const asset fee = core_sym::from_string("0.45600000");
...
BOOST_FIXTURE_TEST_CASE( buy_sell_sell_rex, eosio_system_tester ) try {
   ...
   const int64_t ratio        = rex_ratio;
   const asset   init_balance = core_sym::from_string("40000.00000000");
   const asset   init_net     = core_sym::from_string("70.00000000");
   const asset   init_cpu     = core_sym::from_string("90.00000000");
   const std::vector<account_name> accounts = { N(aliceaccount), N(bobbyaccount), N(carolaccount) };
   account_name alice = accounts[0], bob = accounts[1], carol = accounts[2];
   ...
   // alice lends rex
   const asset payment = core_sym::from_string("30250.00000000");
   BOOST_REQUIRE_EQUAL( success(),              buyrex( alice, payment ) );
   BOOST_REQUIRE_EQUAL( success(),              buyrex( bob, core_sym::from_string("0.00050000") ) );
   ...
   // bob rents cpu for carol
   const asset fee = core_sym::from_string("7.00000000");
   BOOST_REQUIRE_EQUAL( success(),               rentcpu( bob, carol, fee ) );
   rex_pool = get_rex_pool();
   ...
   produce_block( fc::days(5) );
   produce_blocks(2);
   const asset rex_tok = asset::from_string("1.00000000 REX");
...
BOOST_FIXTURE_TEST_CASE( buy_sell_claim_rex, eosio_system_tester ) try {
   ...
   BOOST_REQUIRE_EQUAL( wasm_assert_msg("rex loans are currently not available"),
                        rentcpu( frank, frank, core_sym::from_string("0.00010000") ) );
   {
...

   {
      ...
      BOOST_REQUIRE_EQUAL( wasm_assert_msg("rex loans are currently not available"),
                           rentcpu( frank, frank, core_sym::from_string("1.00000000") ) );
   }
...

   {
      ...
      BOOST_REQUIRE_EQUAL( success(),      buyrex( emily, core_sym::from_string("22000.00000000") ) );
      BOOST_REQUIRE_EQUAL( false,          get_rex_order_obj(alice).is_null() );
      BOOST_REQUIRE_EQUAL( false,          get_rex_order(alice)["is_open"].as<bool>() );

      BOOST_REQUIRE_EQUAL( success(),      rentcpu( frank, frank, core_sym::from_string("1.00000000") ) );
   }
...
BOOST_FIXTURE_TEST_CASE( rex_loans, eosio_system_tester ) try {
   ...
   const int64_t ratio        = rex_ratio;
   const asset   init_balance = core_sym::from_string("40000.00000000");
   const asset   one_unit     = core_sym::from_string("0.00010000");
   const std::vector<account_name> accounts = { N(aliceaccount), N(bobbyaccount), N(carolaccount), N(emilyaccount), N(frankaccount) };
   account_name alice = accounts[0], bob = accounts[1], carol = accounts[2], emily = accounts[3], frank = accounts[4];
   setup_rex_accounts( accounts, init_balance  );

   BOOST_REQUIRE_EQUAL( success(), buyrex( alice, core_sym::from_string("25000.00000000") ) );

   auto rex_pool            = get_rex_pool();
   const asset payment      = core_sym::from_string("30.00000000");
   const asset zero_asset   = core_sym::from_string("0.00000000");
   const asset neg_asset    = core_sym::from_string("-1.00000000");
   ...
   BOOST_REQUIRE_EQUAL( wasm_assert_msg("must use core token"),
                        rentcpu( frank, bob, asset::from_string("10.00000000 RND") ) );
   BOOST_REQUIRE_EQUAL( wasm_assert_msg("must use core token"),
                        rentcpu( frank, bob, payment, asset::from_string("10.00000000 RND") ) );
   ...
   const asset fund = core_sym::from_string("35.00000000");
   ...
   {
      const asset amount = core_sym::from_string("7.50000000");
      ...
      BOOST_REQUIRE_EQUAL( defundcpuloan( frank, 1, core_sym::from_string("75.00000000") ), wasm_assert_msg("insufficent loan balance") );
      old_frank_balance = cur_frank_balance;
   ...
   BOOST_REQUIRE_EQUAL( success(),  buyrex( alice, core_sym::from_string("10.00000000") ) );
...
BOOST_FIXTURE_TEST_CASE( rex_loan_checks, eosio_system_tester ) try {
   ...
   const int64_t ratio        = rex_ratio;
   const asset   init_balance{rex_init_rent*2,core_sym::sym()};
   const std::vector<account_name> accounts = { N(aliceaccount), N(bobbyaccount) };
   account_name alice = accounts[0], bob = accounts[1];
   setup_rex_accounts( accounts, init_balance );

   const asset payment1{rex_init_rent,core_sym::sym()};
   const asset payment2{4'00000000,core_sym::sym()};;
   const asset fee{1'00000000,core_sym::sym()};;
   ...
   const asset asset_rex{rex_init_rent*ratio,symbol{SY(CORE_SYM_PRECISION,REX)}};
   BOOST_REQUIRE_EQUAL( success(),            sellrex( alice, asset_rex ) ) ;
   produce_block( fc::days(31) );
   BOOST_REQUIRE_EQUAL( success(),            rexexec( alice, 3) );
   BOOST_REQUIRE_EQUAL( true,                 get_cpu_loan(1).is_null() );
...
BOOST_FIXTURE_TEST_CASE( ramfee_namebid_to_rex, eosio_system_tester ) try {
   ...
   const std::vector<account_name> accounts = { N(aliceaccount), N(bobbyaccount), N(carolaccount), N(emilyaccount), N(frankaccount) };
   account_name alice = accounts[0], bob = accounts[1], carol = accounts[2], emily = accounts[3], frank = accounts[4];
   setup_rex_accounts( accounts, init_balance, core_sym::from_string("80.00000000"), core_sym::from_string("80.00000000"), false );

   asset cur_ramfee_balance = get_balance( N(eosio.ramfee) );
   BOOST_REQUIRE_EQUAL( success(),                      buyram( alice, alice, core_sym::from_string("20.00000000") ) );
   BOOST_REQUIRE_EQUAL( get_balance( N(eosio.ramfee) ), core_sym::from_string("0.10000000") + cur_ramfee_balance );
   BOOST_REQUIRE_EQUAL( wasm_assert_msg("must deposit to REX fund first"),
                        buyrex( alice, core_sym::from_string("350.00000000") ) );
   BOOST_REQUIRE_EQUAL( success(),                      deposit( alice, core_sym::from_string("350.00000000") ) );
   BOOST_REQUIRE_EQUAL( success(),                      buyrex( alice, core_sym::from_string("350.00000000") ) );
   cur_ramfee_balance = get_balance( N(eosio.ramfee) );
   asset cur_rex_balance = get_balance( N(eosio.rex) );
   BOOST_REQUIRE_EQUAL( core_sym::from_string("350.00000000"), cur_rex_balance );
   BOOST_REQUIRE_EQUAL( success(),                         buyram( bob, carol, core_sym::from_string("70.00000000") ) );
   BOOST_REQUIRE_EQUAL( cur_ramfee_balance,                get_balance( N(eosio.ramfee) ) );
   BOOST_REQUIRE_EQUAL( get_balance( N(eosio.rex) ),       cur_rex_balance + core_sym::from_string("0.35000000") );
   ...
   cur_rex_balance = get_balance( N(eosio.rex) );
   BOOST_REQUIRE_EQUAL( success(),                        bidname( carol, N(rndmbid), core_sym::from_string("23.70000000") ) );
   BOOST_REQUIRE_EQUAL( core_sym::from_string("23.70000000"), get_balance( N(eosio.names) ) );
   BOOST_REQUIRE_EQUAL( success(),                        bidname( alice, N(rndmbid), core_sym::from_string("29.35000000") ) );
   BOOST_REQUIRE_EQUAL( core_sym::from_string("29.35000000"), get_balance( N(eosio.names) ));

   produce_block( fc::hours(24) );
   produce_blocks( 2 );

   BOOST_REQUIRE_EQUAL( core_sym::from_string("29.35000000"), get_rex_pool()["namebid_proceeds"].as<asset>() );
   BOOST_REQUIRE_EQUAL( success(),                        deposit( frank, core_sym::from_string("5.00000000") ) );
   BOOST_REQUIRE_EQUAL( success(),                        buyrex( frank, core_sym::from_string("5.00000000") ) );
   BOOST_REQUIRE_EQUAL( get_balance( N(eosio.rex) ),      cur_rex_balance + core_sym::from_string("34.35000000") );
...
BOOST_FIXTURE_TEST_CASE( rex_maturity, eosio_system_tester ) try {
   ...
   const symbol  rex_sym( SY(CORE_SYM_PRECISION, REX) );

   {
      // alice buy 11.5
      BOOST_REQUIRE_EQUAL( success(), buyrex( alice, core_sym::from_string("11.50000000") ) );
      produce_block( fc::hours(3) );

      // alice buy 18.5
      BOOST_REQUIRE_EQUAL( success(), buyrex( alice, core_sym::from_string("18.50000000") ) );
      produce_block( fc::hours(25) );

      // alice buy 25
      BOOST_REQUIRE_EQUAL( success(), buyrex( alice, core_sym::from_string("25.00000000") ) );

      auto rex_balance = get_rex_balance_obj( alice );
      // alice total bought 55. -> 5500000000
      BOOST_REQUIRE_EQUAL( 5500000000 * rex_ratio, rex_balance["rex_balance"].as<asset>().get_amount() );
      BOOST_REQUIRE_EQUAL( 0,                  rex_balance["matured_rex"].as<int64_t>() );
      BOOST_REQUIRE_EQUAL( 2,                  rex_balance["rex_maturities"].get_array().size() );

      // in first day ,  alice 's rex can't sell.
      const asset amt_11_5 = core_sym::from_string("11.50000000");
      const asset rex_11_5 ( rex_ratio * amt_11_5.get_amount(), rex_sym );
      BOOST_REQUIRE_EQUAL( wasm_assert_msg("insufficient available rex"),
                           sellrex( alice, rex_11_5 ) );
      // pass 3 days.
      produce_block( fc::hours( 3*24 + 20) );
      // alice sell 30.
      const asset amt_30 = core_sym::from_string("30.00000000");
      const asset rex_30 ( rex_ratio * amt_30.get_amount(), rex_sym );
      BOOST_REQUIRE_EQUAL( success(),          sellrex( alice, rex_30 ) );

      rex_balance = get_rex_balance_obj( alice );
      BOOST_REQUIRE_EQUAL( 2500000000 * rex_ratio, rex_balance["rex_balance"].as<asset>().get_amount() );
      BOOST_REQUIRE_EQUAL( 0,                  rex_balance["matured_rex"].as<int64_t>() );
      BOOST_REQUIRE_EQUAL( 1,                  rex_balance["rex_maturities"].get_array().size() );
      // in 4th day, alice 's rest 25 can't sell.
      produce_block( fc::hours(23) );
      const asset amt_25 = core_sym::from_string("25.00000000");
      const asset rex_25 ( rex_ratio * amt_25.get_amount(), rex_sym );
      BOOST_REQUIRE_EQUAL( wasm_assert_msg("insufficient available rex"),
                           sellrex( alice, rex_25 ) );
      produce_block( fc::days(1) );
      //in 5th day, alice can sell 13.
      const asset amt_13 = core_sym::from_string("13.00000000");
      const asset rex_13 ( rex_ratio * amt_13.get_amount(), rex_sym );
      BOOST_REQUIRE_EQUAL( success(),          sellrex( alice, rex_13 ) );

      rex_balance = get_rex_balance_obj( alice );
      BOOST_REQUIRE_EQUAL( 1200000000* rex_ratio,         rex_balance["rex_balance"].as<asset>().get_amount() );
      BOOST_REQUIRE_EQUAL( 1200000000* rex_ratio,         rex_balance["matured_rex"].as<int64_t>() );
      BOOST_REQUIRE_EQUAL( 0,                  rex_balance["rex_maturities"].get_array().size() );
      // alice balance only 55 -30 -13 = 12. 
      BOOST_REQUIRE_EQUAL( wasm_assert_msg("insufficient available rex"),
                           sellrex( alice, rex_13 ) );

      const asset amt_12 = core_sym::from_string("12.00000000");
      const asset rex_12 ( rex_ratio * amt_12.get_amount(), rex_sym );
      BOOST_REQUIRE_EQUAL( success(),          sellrex( alice, rex_12 ) );

      rex_balance = get_rex_balance_obj( alice );
      // alice's rex all sold.
   ...
   {
      // bob have 10000 as initial.
      const asset payment1 = core_sym::from_string("14.80000000");
      const asset payment2 = core_sym::from_string("15.20000000");
      const asset payment  = payment1 + payment2;
      const asset rex_bucket( rex_ratio * payment.get_amount(), rex_sym );
      // bob total bought 30*8=240
   ...
   {
      const asset payment1 = core_sym::from_string("20000.00000000");
      const asset payment2 = core_sym::from_string("1000.00000000");
      ...
      BOOST_REQUIRE_EQUAL( success(),                rentcpu( alice, alice, core_sym::from_string("10.00000000") ) );
...
BOOST_FIXTURE_TEST_CASE( rex_savings, eosio_system_tester ) try {
   ...
   const asset init_balance = core_sym::from_string("100000.00000000");
   const std::vector<account_name> accounts = { N(aliceaccount), N(bobbyaccount), N(carolaccount), N(emilyaccount), N(frankaccount) };
   account_name alice = accounts[0], bob = accounts[1], carol = accounts[2], emily = accounts[3], frank = accounts[4];
   setup_rex_accounts( accounts, init_balance );
...

   {
      ...
      BOOST_REQUIRE_EQUAL( wasm_assert_msg("insufficient available rex"),
                           sellrex( alice, asset::from_string( "1.00000000 REX" ) ) );
      BOOST_REQUIRE_EQUAL( success(),                   mvfrsavings( alice, asset::from_string( "10.00000000 REX" ) ) );
      rex_balance = get_rex_balance_obj( alice );
      BOOST_REQUIRE_EQUAL( 2,                           rex_balance["rex_maturities"].get_array().size() );
      produce_block( fc::days(3) );
      BOOST_REQUIRE_EQUAL( wasm_assert_msg("insufficient available rex"),
      ...
      produce_block( fc::days(2) );
      BOOST_REQUIRE_EQUAL( wasm_assert_msg("insufficient available rex"),
                           sellrex( alice, asset::from_string( "10.00010000 REX" ) ) );
      BOOST_REQUIRE_EQUAL( success(),                   sellrex( alice, asset::from_string( "10.00000000 REX" ) ) );
      rex_balance = get_rex_balance_obj( alice );
      BOOST_REQUIRE_EQUAL( 1,                           rex_balance["rex_maturities"].get_array().size() );
      produce_block( fc::days(100) );
      BOOST_REQUIRE_EQUAL( wasm_assert_msg("insufficient available rex"),
                           sellrex( alice, asset::from_string( "0.00010000 REX" ) ) );
   ...
   {
      const asset payment = core_sym::from_string("20.00000000");
      const asset rex_bucket( rex_ratio * payment.get_amount(), rex_sym );
...

   {
      ...
      BOOST_REQUIRE_EQUAL( success(),                       rentcpu( bob, bob, core_sym::from_string("3000.00000000") ) );
   ...
   {
      const asset   payment                = core_sym::from_string("250.00000000");
      const asset   half_payment           = core_sym::from_string("125.00000000");
      const int64_t rex_bucket_amount      = rex_ratio * payment.get_amount();
      ...
      produce_block( fc::days(5) );
      BOOST_REQUIRE_EQUAL( wasm_assert_msg("asset must be a positive amount of (REX, 8)"),
                           mvfrsavings( carol, asset::from_string("0.00000000 REX") ) );
      BOOST_REQUIRE_EQUAL( wasm_assert_msg("asset must be a positive amount of (REX, 8)"),
                           mvfrsavings( carol, asset::from_string("1.00000000 RND") ) );
      BOOST_REQUIRE_EQUAL( success(),                   mvfrsavings( carol, half_rex_bucket ) );
      BOOST_REQUIRE_EQUAL( wasm_assert_msg("insufficient REX in savings"),
                           mvfrsavings( carol, asset::from_string("0.00010000 REX") ) );
      rex_balance = get_rex_balance_obj( carol );
      BOOST_REQUIRE_EQUAL( 1,                           rex_balance["rex_maturities"].get_array().size() );
...
BOOST_FIXTURE_TEST_CASE( update_rex, eosio_system_tester, * boost::unit_test::tolerance(1e-10) ) try {
   ...
   // alice buys rex
   const asset payment = core_sym::from_string("25000.00000000");
   BOOST_REQUIRE_EQUAL( success(),                              buyrex( alice, payment ) );
   ...
   // emily rents cpu
   const asset fee = core_sym::from_string("50.00000000");
   ...
   const asset rex_sell_amount( get_rex_balance(alice).get_amount() / 4, symbol( SY(CORE_SYM_PRECISION,REX) ) );
   ...
   BOOST_REQUIRE_EQUAL( wasm_assert_msg("must vote for at least 21 producers or for a proxy before buying REX"),
                        buyrex( alice, core_sym::from_string("1.00000000") ) );
...
BOOST_FIXTURE_TEST_CASE( update_rex_vote, eosio_system_tester, * boost::unit_test::tolerance(1e-8) ) try {
   ...
   const asset purchase = core_sym::from_string("25000.00000000");
   ...
   const auto init_rex_pool = get_rex_pool();
   const asset rent = core_sym::from_string("25.00000000");
   ...
   const asset to_net_stake = core_sym::from_string("60.00000000");
   const asset to_cpu_stake = core_sym::from_string("40.00000000");
...
BOOST_FIXTURE_TEST_CASE( deposit_rex_fund, eosio_system_tester ) try {
   ...
   BOOST_REQUIRE_EQUAL( core_sym::from_string("0.00000000"),                   get_rex_fund( alice ) );
   BOOST_REQUIRE_EQUAL( wasm_assert_msg("must deposit to REX fund first"), withdraw( alice, core_sym::from_string("0.00010000") ) );
   BOOST_REQUIRE_EQUAL( wasm_assert_msg("overdrawn balance"),              deposit( alice, init_balance + init_balance ) );
   BOOST_REQUIRE_EQUAL( wasm_assert_msg("must deposit core token"),        deposit( alice, asset::from_string("1.00000000 RNDM") ) );
   ...
   BOOST_REQUIRE_EQUAL( wasm_assert_msg("insufficient funds"), withdraw( alice, get_rex_fund( alice ) + core_sym::from_string("0.00010000")) );
...
BOOST_FIXTURE_TEST_CASE( rex_lower_bound, eosio_system_tester ) try {
   ...
   const asset init_balance = core_sym::from_string("25000.00000000");
   const std::vector<account_name> accounts = { N(aliceaccount), N(bobbyaccount) };
   account_name alice = accounts[0], bob = accounts[1];
   setup_rex_accounts( accounts, init_balance );
   const symbol rex_sym( SY(CORE_SYM_PRECISION, REX) );
   ...
   const asset payment = core_sym::from_string("25000.00000000");
   BOOST_REQUIRE_EQUAL( success(), buyrex( alice, payment ) );
   const asset fee = core_sym::from_string("25.00000000");
...
BOOST_FIXTURE_TEST_CASE( close_rex, eosio_system_tester ) try {
   ...
   const asset fee = core_sym::from_string("1.00000000");
   ...
   BOOST_REQUIRE_EQUAL( wasm_assert_msg("rex loans are currently not available"),
                        rentcpu( emily, emily, core_sym::from_string("1.00000000") ) );
...
BOOST_FIXTURE_TEST_CASE( set_rex, eosio_system_tester ) try {
   ...
   // both a and b have 25k.
   setup_rex_accounts( accounts, init_balance );

   const name act_name{ N(setrex) };
   const asset init_total_rent{rex_init_rent,core_sym::sym()};
   const asset set_total_rent{rex_init_rent/2,core_sym::sym()};
   const asset negative_balance{rex_init_rent/2*(-1), core_sym::sym()};
   const asset different_symbol{rex_init_rent/2,symbol{SY(CORE_SYM_PRECISION,RND)}};

   // only eosio can call setrex.
   BOOST_REQUIRE_EQUAL( error("missing authority of eosio"),
                        push_action( alice, act_name, mvo()("balance", set_total_rent) ) );
   BOOST_REQUIRE_EQUAL( error("missing authority of eosio"),
                        push_action( bob, act_name, mvo()("balance", set_total_rent) ) );
   // can't setrex before init.
   ...
   const asset fee = core_sym::from_string("10.00000000");
   // B rentcpu 10.
   BOOST_REQUIRE_EQUAL( success(),             rentcpu( bob, bob, fee ) );
...
// B1 vesting will remove from axis contract.  
// rem out temperatory.
...
//    issue_and_transfer( alice, core_sym::from_string("20000.00000000"), config::system_account_name );
//    issue_and_transfer( bob,   core_sym::from_string("20000.00000000"), config::system_account_name );
//    BOOST_REQUIRE_EQUAL( success(), bidname( bob,   b1, core_sym::from_string( "0.50000000" ) ) );
//    BOOST_REQUIRE_EQUAL( success(), bidname( alice, b1, core_sym::from_string( "1.00000000" ) ) );

//    produce_block( fc::days(1) );

//    create_accounts_with_resources( { b1 }, alice );

//    const asset stake_amount = core_sym::from_string("5000.00000000");
//    const asset half_stake   = core_sym::from_string("2500.00000000");
//    const asset small_amount = core_sym::from_string("1000.00000000");
...
//    BOOST_REQUIRE_EQUAL( success(), rentcpu( bob, bob, core_sym::from_string("10000.00000000") ) );
...
BOOST_FIXTURE_TEST_CASE( rex_return, eosio_system_tester ) try {
   ...
   const asset payment = core_sym::from_string("100000.00000000");
   {
      BOOST_REQUIRE_EQUAL( success(),        buyrex( alice, payment ) );
      auto rex_pool = get_rex_pool();
   ...
   {
      const asset    fee                 = core_sym::from_string("30.00000000");
...

   {
      ...
      const asset fee = core_sym::from_string("15.00000000");
      BOOST_REQUIRE_EQUAL( success(),        rentcpu( bob, bob, fee ) );
      auto rex_return_pool = get_rex_return_pool();
...
BOOST_FIXTURE_TEST_CASE( change_limited_account_back_to_unlimited, eosio_system_tester ) try {
   ...
   transfer( N(eosio), N(alice1111111), core_sym::from_string("1.00000000") );

   auto error_msg = stake( N(alice1111111), N(eosio), core_sym::from_string("0.00000000"), core_sym::from_string("1.00000000") );
   ...
   stake( N(alice1111111), N(eosio), core_sym::from_string("0.00000000"), core_sym::from_string("1.00000000") );

   REQUIRE_MATCHING_OBJECT( get_total_stake( "eosio" ), mvo()
      ("owner", "eosio")
      ("net_weight", core_sym::from_string("0.00000000"))
      ("cpu_weight", core_sym::from_string("1.00000000"))
      ("ram_bytes",  0)
   );
...
BOOST_FIXTURE_TEST_CASE( buy_pin_sell_ram, eosio_system_tester ) try {
   ...
   transfer( N(eosio), N(alice1111111), core_sym::from_string("1020.00000000") );

   auto error_msg = stake( N(alice1111111), N(eosio), core_sym::from_string("10.00000000"), core_sym::from_string("10.00000000") );
   auto semicolon_pos = error_msg.find(';');
   ...
   REQUIRE_MATCHING_OBJECT( total_res, mvo()
      ("owner", "eosio")
      ("net_weight", core_sym::from_string("0.00000000"))
      ("cpu_weight", core_sym::from_string("0.00000000"))
